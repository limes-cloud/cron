...
func (c *Cron) run() {
	...
	for _, entry := range c.entries {
		...
		c.logger.Infow("type", "schedule", "now", now, "entry", entry.ID, "next", entry.Next)
	}
	...
	for {
		...
		for {
			...
			case now = <-timer.C:
				...
				c.logger.Infow("type", "wake", "now", now)
				...
				for _, e := range c.entries {
					...
					c.logger.Infow("type", "run", "now", now, "entry", e.ID, "next", e.Next)
				}
			...
			case newEntry := <-c.add:
				...
				c.logger.Infow("type", "added", "now", now, "entry", newEntry.ID, "next", newEntry.Next)
			...
			case id := <-c.remove:
				...
				now = c.now()
				c.removeEntry(id)
				c.logger.Infow("type", "removed", "entry", id)
			}
...
func (c *Cron) removeEntry(id uint32) {
	var entries []*Entry
	for _, e := range c.entries {
		if e.ID != id {
